...
var templates = template.Must(template.New("main").Funcs(template.FuncMap{
	"html": func(b []byte) template.HTML {
		return template.HTML(b)
	},
}).ParseFiles("edit.html", "view.html"))
...
func createHTMLPageLinks(p *Page) {
	pL := regexp.MustCompile(`\[([a-zA-Z]+)\]`)
	p.Body = pL.ReplaceAllFunc(p.Body, func(s []byte) []byte {
		group := pL.ReplaceAllString(string(s), "$1")
